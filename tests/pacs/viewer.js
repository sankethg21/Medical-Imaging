...
function runRenderTest() {
  const numberOfRun = 20;
  // Ensure _app is defined and has the method setActiveLayerGroup
  if (typeof _app === 'undefined' || typeof _app.setActiveLayerGroup !== 'function') {
    console.error('_app is undefined or does not have setActiveLayerGroup method');
    return;
  }
  ...
  // Ensure getActiveLayerGroup returns a valid object with getActiveViewLayer method
  const layerGroup = _app.getActiveLayerGroup();
  if (typeof layerGroup === 'undefined' || typeof layerGroup.getActiveViewLayer !== 'function') {
    console.error('Active layer group is undefined or does not have getActiveViewLayer method');
    return;
  }

  const vl = layerGroup.getActiveViewLayer();
  if (typeof vl === 'undefined') {
    console.error('No active view layer found');
    return;
  }

  const vc = vl.getViewController();
  if (typeof vc === 'undefined' || typeof vc.incrementScrollIndex !== 'function') {
    console.error('ViewController is undefined or does not have incrementScrollIndex method');
    return;
  }
